...
<div class="content-box"><!-- Start Content Box -->
    ...
    <div class="content-box-content">
        <form action="admin/categories/action" method="post">
            <input type="hidden" name="redirect" value="<?= base64_encode($this->my_string->fullurl())?>">
        <?php
            ...
            ?>
                    <tr>
                        <td><input type="checkbox" name="checkbox[]" value="<?= $row['id']?>"/></td>
                            ...
                            <td hidden> <?= $row['id'] ?> </td>
                            <td> <?= cutnchar($row['title'], 20) ?> </td>
                            <td> <?= cutnchar($row['description'], 20) ?> </td>
                        <td>
                            <a href="categories/togglepushlish/publish/<?= $row['id']?>/<?= ($row['publish'] == 'published')? 'unpublished' : 'published' ?>?r=<?= base64_encode($this->my_string->fullurl())?>">
                        ...
                        <td>
                            <!-- Icons -->
                            <a href="categories/edit?id=<?= $row['id']?>&r=<?= base64_encode($this->my_string->fullurl()) ?>" title="Edit">
                                <img src="public/simpla-admin/resources/images/icons/pencil.png" alt="Edit"/>
                            </a>
                            <a href="categories/delete/<?= $row['id']?>?r=<?= base64_encode($this->my_string->fullurl()) ?>" title="Delete">
